function output = render_volumes(res, bounds, phi, X, Y, Z, U, V, W, lambda)
    output={};
    for i=1:numel(bounds)
        ...
        output{i}={x_grid, y_grid, z_grid, abs(gather(pressure_field(phi,X,Y,Z,U,V,W,x_grid,y_grid,z_grid, lambda)))};
    end
end